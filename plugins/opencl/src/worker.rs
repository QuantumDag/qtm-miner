...
static PROGRAM_SOURCE: &str = include_str!("../resources/qtm-opencl.cl");
    ...
    pub fn new(
        ...
        let program = match use_binary {
            true => {
                ...
                match BINARY_DIR.get_file(format!("{}_qtm-opencl.bin", device_name)) {
                    Some(binary) => {
                        Program::create_and_build_from_binary(&context, &[binary.contents()], "").unwrap_or_else(|e|{
                        //Program::create_and_build_from_binary(&context, &[include_bytes!("../resources/qtm-opencl-linked.bc")], "").unwrap_or_else(|e|{
