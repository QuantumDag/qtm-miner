...
use crate::proto::qtmd_message::Payload;
use crate::proto::rpc_client::RpcClient;
use crate::proto::{
    GetBlockTemplateRequestMessage, GetInfoRequestMessage, QtmdMessage, NotifyBlockAddedRequestMessage,
...
type BlockHandle = JoinHandle<Result<(), PollSendError<QtmdMessage>>>;

#[allow(dead_code)]
pub struct QtmdHandler {
    client: RpcClient<TonicChannel>,
    pub send_channel: Sender<QtmdMessage>,
    stream: Streaming<QtmdMessage>,
    ...
    async fn listen(&mut self, miner: &mut MinerManager) -> Result<(), Error> {
        while let Some(msg) = self.stream.message().await? {
            match msg.payload {
                ...
                None => warn!("qtm message payload is empty"),
            }
        }
        Ok(())
    }
...
impl QtmdHandler {
    ...
    fn create_block_channel(send_channel: Sender<QtmdMessage>) -> (Sender<BlockSeed>, BlockHandle) {
        // QtmdMessage::submit_block(block)
            ...
            tokio::spawn(async move {
                ReceiverStream::new(recv)
                    .map(|block_seed| match block_seed {
                        FullBlock(block) => QtmdMessage::submit_block(*block),
    ...
    async fn client_send(&self, msg: impl Into<QtmdMessage>) -> Result<(), SendError<QtmdMessage>> {
        self.send_channel.send(msg.into()).await
    }

    async fn client_get_block_template(&mut self) -> Result<(), SendError<QtmdMessage>> {
    ...
    async fn handle_message(&mut self, msg: Payload, miner: &mut MinerManager) -> Result<(), Error> {
        match msg {
            ...
            Payload::GetInfoResponse(info) => {
                info!("Qtmd version: {}", info.server_version);
                let qtm_version = Version::parse(&info.server_version)?;
                let update_version = Version::parse(VERSION_UPDATE)?;
                match qtm_version >= update_version {
            ...
            Payload::NotifyBlockAddedResponse(res) => match res.error {
                None => info!("Registered for block notifications (upgrade your Qtmd for better experience)"),
