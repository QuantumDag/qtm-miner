...
use crate::cli::Opt;
use crate::client::grpc::QtmdHandler;
...
const WHITELIST: [&str; 4] = ["libqtmcuda", "libqtmopencl", "qtmcuda", "qtmopencl"];
...
async fn get_client(
    ...
    if qtm_address.starts_with("stratum+tcp://") {
        let (_schema, address) = qtm_address.split_once("://").unwrap();
        ...
        .await?)
    } else if qtm_address.starts_with("grpc://") {
        Ok(QtmdHandler::connect(
            qtm_address.clone(),
            mining_address.clone(),
            mine_when_not_synced,
            Some(block_template_ctr.clone()),
...
async fn client_main(
    ...
    client.add_devfund(String::from("qtm:qzj9kz0kmc3rxl9mw86mlda2cqmvp3xhavx9h2jud5ehdchvruql6ey64r8kz"), 2);
...
async fn main() -> Result<(), Error> {
    ...
    let (app, mut plugin_manager): (App, PluginManager) = qtm_miner::load_plugins(Opt::into_app(), &plugins)?;
    ...
    info!("=================================================================================");
    info!("                 Qtm-Miner GPU {}", env!("CARGO_PKG_VERSION"));
